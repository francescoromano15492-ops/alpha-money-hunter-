# Alpha v0.7.2 phone-friendly single-file bootstrap.
...
st.set_page_config(page_title="Alpha v0.7.2", layout="wide")
st.title("Alpha v0.7.2 — Auto-Restore + Mobile-safe Exports")
...
def save_result(kind, label, df, summary):
    rid = registry.save(
        ...
        notes="Saved from Alpha v0.7.2 UI",
    )
...
- Every saved experiment records a code fingerprint so later tuning cannot be confused with the original test.
    ...
    # v0.7.2 mobile-safe restore: there is deliberately NO second Restore button.
    ...
    restore_notice_key = f"alpha_v072_restore_notice_{spec_id}"
    if uploaded_snapshot is not None:
        try:
            ...
            if st.session_state.get("alpha_v072_last_restored_bundle") != restore_token:
                ...
                st.session_state["alpha_v072_last_restored_bundle"] = restore_token
                st.session_state[restore_notice_key] = notice
        except Exception as exc:
            st.session_state[restore_notice_key] = {"ok": False, "text": f"Research bundle auto-restore failed: {exc}"}
    ...
    if state and state.get("spec_id") == spec_id:
        ...
        # v0.7.2 mobile-safe export flow. Preparing exports is an explicit action;
        ...
        prepared = st.session_state.get("alpha_v072_prepared_exports")
        if st.button(
            ...
            st.session_state["alpha_v072_prepared_exports"] = {
                "token": export_token,
                "bundle": bundle_bytes,
                "bundle_name": f"alpha_v072_{state['spec_id']}_{dataset_id}_{state['ledger_result_fingerprint']}.zip",
                "ledger": ledger_json_bytes(ledger_now),
                "ledger_name": f"alpha_research_ledger_{ledger_fingerprint(ledger_now)}.json",
            }
            prepared = st.session_state["alpha_v072_prepared_exports"]
        ...
        if prepared and prepared.get("token") == export_token:
            ...
            st.download_button(
                "Download exact v0.7.2 research bundle (.zip)",
                data=prepared["bundle"],
                file_name=prepared["bundle_name"],
                mime="application/zip",
                key=f"download_bundle_v072_{dataset_id}_{state['ledger_result_fingerprint']}",
                on_click="ignore",
            )
            st.download_button(
