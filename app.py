# Alpha v0.7 phone-friendly single-file bootstrap.
...
st.set_page_config(page_title="Alpha v0.7", layout="wide")
st.title("Alpha v0.7 — Cumulative Research Ledger")
...
def save_result(kind, label, df, summary):
    rid = registry.save(
        ...
        notes="Saved from Alpha v0.7 UI",
    )
...
- Every saved experiment records a code fingerprint so later tuning cannot be confused with the original test.
    ...
    if state and state.get("spec_id") == spec_id:
        ...
        st.download_button(
            "Download exact v0.7 research bundle (.zip)",
            data=bundle_bytes,
            file_name=f"alpha_v07_{state['spec_id']}_{dataset_id}_{state['ledger_result_fingerprint']}.zip",
            mime="application/zip",
            key=f"download_bundle_{dataset_id}_{state['ledger_result_fingerprint']}",
        )
        st.download_button(
            ...
            key=f"download_ledger_{ledger_fingerprint(ledger_now)}",
        )
        st.caption(
